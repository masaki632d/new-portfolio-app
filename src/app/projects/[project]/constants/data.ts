...
export const projectsData = [
  {
    ...
    mainProjectText: [
      ...
      '<b>苦労したこと</b><br><b>[TailwindCSSのインライン記法への適応と既存スタイルの再現]</b><br>TailwindCSSのユーティリティクラスを使用した インラインstyling への移行は、従来のCSS Modules や プレーンなCSSクラス とは異なるアプローチで最初は慣れが必要でした。特に、複雑なレイアウトやコンポーネントにおいては、既存のスタイルを再現しつつ TailwindCSS 特有の記法に適応するために多くの試行錯誤が必要でした。<br>リプレイスするページ数の物量が多かったですが、ページ種別ごとに切り分けて洗い出し、着実に進捗を出せました。<br><br><b>[大量のページリプレイス作業と管理]</b><br>サイト全体のリプレイスにおいて、多数のページが対象となり、それぞれのページごとに異なる要件やデザインが存在していました。このため、各ページを効率的に分類し、優先順位をつけてリプレイス作業を進めることが重要でした。<br>作業の進捗を管理しながら、デザイナーや他の開発者と協力して、全ページのリプレイスを期限内に完了させるために、プロジェクト管理のスキルも高められました。<br><br><b>[社内の技術スタック変更に対する教育とサポート]</b><br>TailwindCSSの導入にあたって、チームメンバー全員が新しいコーディング規約に従うよう、教育やサポートも必要でした。特に、CSS Modules や プレーンなCSS に慣れていたメンバーが新しい TailwindCSS の記法に適応するためのトレーニングや、ベストプラクティスを共有するためのドキュメント作成なども行いました。これにより、チーム全体が一貫したコーディングスタイルを維持し、プロジェクトの保守性と効率性を向上させることができました。<br>',

      '<b>成果</b><br>作成したコーディング規約の運用 と TailwindCSSへのリプレイスによって、CSSの二重読み込みの負債が解消。<br>表示崩れによる機会損失の回避・開発運用コスト削減・不要CSS削減による Performance改善 に繋がりました。',
    ],
    keyExperienceText: [
  ...
  },
  {
    ...
    technologyText: [
      '<b>リプレイス後</b><br>Next.js / React / TypeScript / Emotion / axios / Prettier / ESLint / lint-staged / husky / Git<br>Ruby on Rails(バックエンドAPI構築)',
      '<b>リプレイス前</b><br>WordPress(コンテンツ管理システム) / Sass(スタイリング)<br>バックエンドは Symfony(PHPフレームワーク)',
      'Figma / Photoshop / Illustrator',
    ],
    problemText: [
      '<b>技術的課題</b><br><b>[Legacy技術からの移行]</b><br>固有の構成の都合により、社内共通ライブラリ機能 の恩恵を受けることができない状況がありました。<br>また、WordPress と Symfony を使い続けることによるセキュリティリスクや、パフォーマンスの低下が見られ、<br>そして老朽化した構成 により改修が困難になっていました。<br><br><b>[CSSのスパゲッティコードの管理]</b><br>既存のCSS は複雑で管理が難しく、デザイン変更が困難になっている状況でした。<br><br><b>[デザインの二重管理]</b><br>PCとSPの別々のデザインの管理による、運用コストの増大がありました。',

      '<b>その他の問題</b><br>古い技術の継続使用による、人材採用悪化の懸念がありました。',
    ],
    mainProjectText: [
      '<b>SEOサイトのReact⼤規模リプレイス & Lighthouseスコア改善 PJ</b><br>このプロジェクトでは 上記の課題解決 を目的として、WordPressとSymfonyで運用されていたサイトを、よりモダンで柔軟性のある Next.js と React にリプレイスし、パフォーマンスとセキュリティ、またLighthouseのスコア を向上させることを目指しました。（※並行して、リプレイスPJ以外のタスクも対応）',
      ...
      '<b>その他の成果</b><br>デザイナー組織内で、Reactのメリットや実装方法、Atomic Design の設計方法などを社内共有会 で展開し、社内のデザイナーに対して知見を広める動きで貢献することができました。',
    ],
    keyExperienceText: [
